...
Objective setting & planning (what to do this cycle)

Risk assessment & reduction (identify/mitigate risks)

Development & validation (build + test)

Review & plan next cycle (customer evaluation)
...
Define software. List its characteristics.
Programs + data + docs; intangible, developed not manufactured, doesn’t wear out, complex, changeable…

Explain Waterfall model with diagram and pros/cons.
Linear phases; simple but rigid; late testing…

Differentiate Spiral and Prototyping.
Spiral = risk-driven cycles; Prototyping = quick model for feedback.

What is project management? Explain triple constraint.
Scope–Time–Cost trade-off; quality impact.

What are software metrics? Explain LOC, FP, and Cyclomatic Complexity.

What are Fourth Generation Techniques? Give examples and advantages.
